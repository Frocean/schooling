...
void exchange(int arr1[], int arr2[], int &lx, int &ly) {
    int maxSize = max(lx, ly) + 1;
    int* bot = new int[maxSize];
    memcpy(bot, arr1, (lx + 1) * sizeof(int));
    memcpy(arr1, arr2, (ly + 1) * sizeof(int));
    memcpy(arr2, bot, (lx + 1) * sizeof(int));
    swap(lx, ly);
    delete[] bot;
}
...
void TMainForm::slot_ans() {
	...
	if (s_x -> currentItem() == -1) x_index = 2;
	if (s_y -> currentItem() == -1) y_index = 2;
	if (s_z -> currentItem() == -1) z_index = 2;
	for (int i = 1 ; i <= x_len ; ++ i) {
		if (('a' <= x_char[i]) && (x_char[i] < 'g'))
			x_char[i] -= 'a' - 'A'; //transfer a to A
		if (!judge(x_char[i], x_index)) {
			o_ans -> setText("CE: illegal number"); return;
		}
	}
	...
	switch (op_index) {
		...
		default:o_ans -> setText("CE: unknown operation(1)");return;
	}
	if (op_index == 1)
		if (cmp(s1, s2, x_len, y_len))
			sym = sym ^ 1, exchange(s1, s2, x_len, y_len);
...
//	return;
		...
		case 3:
			if (!y_len || (y_len == 1 && y_char[1] == '0')) {
				o_ans -> setText("CE: divide by zero, you're suck in math");return;
			} else {
			...
			break;
		default:o_ans -> setText("CE: unknown operation(2)");return;
	}
	op_index = oop_index;
}
void TMainForm::k0_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'0');break;
		case 2:text = i_y -> text();i_y -> setText(text+'0');break;
	}
}
void TMainForm::k1_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'1');break;
		case 2:text = i_y -> text();i_y -> setText(text+'1');break;
		default:o_ans -> setText("Input Error");break;
	}
}
void TMainForm::k2_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'2');break;
		case 2:text = i_y -> text();i_y -> setText(text+'2');break;
		default:o_ans -> setText("Input Error");break;
	}
}
void TMainForm::k3_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'3');break;
		case 2:text = i_y -> text();i_y -> setText(text+'3');break;
		default:o_ans -> setText("Input Error");break;
	}
}
void TMainForm::k4_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'4');break;
		case 2:text = i_y -> text();i_y -> setText(text+'4');break;
		default:o_ans -> setText("Input Error");break;
	}
}
void TMainForm::k5_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'5');break;
		case 2:text = i_y -> text();i_y -> setText(text+'5');break;
		default:o_ans -> setText("Input Error");break;
	}
}
void TMainForm::k6_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'6');break;
		case 2:text = i_y -> text();i_y -> setText(text+'6');break;
		default:o_ans -> setText("Input Error");break;
	}
}
void TMainForm::k7_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'7');break;
		case 2:text = i_y -> text();i_y -> setText(text+'7');break;
		default:o_ans -> setText("Input Error");break;
	}
}
void TMainForm::k8_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'8');break;
		case 2:text = i_y -> text();i_y -> setText(text+'8');break;
		default:o_ans -> setText("Input Error");break;
	}
}
void TMainForm::k9_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'9');break;
		case 2:text = i_y -> text();i_y -> setText(text+'9');break;
		default:o_ans -> setText("Input Error");break;
	}
}
void TMainForm::ka_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'A');break;
		case 2:text = i_y -> text();i_y -> setText(text+'A');break;
		default:o_ans -> setText("Input Error");break;
	}
}
void TMainForm::kb_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'B');break;
		case 2:text = i_y -> text();i_y -> setText(text+'B');break;
		default:o_ans -> setText("Input Error");break;
	}
}
void TMainForm::kc_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'C');break;
		case 2:text = i_y -> text();i_y -> setText(text+'C');break;
		default:o_ans -> setText("Input Error");break;
	}
}
void TMainForm::kd_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'D');break;
		case 2:text = i_y -> text();i_y -> setText(text+'D');break;
		default:o_ans -> setText("Input Error");break;
	}
}
void TMainForm::ke_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'E');break;
		case 2:text = i_y -> text();i_y -> setText(text+'E');break;
		default:o_ans -> setText("Input Error");break;
	}
}
void TMainForm::kf_slot() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text+'F');break;
		case 2:text = i_y -> text();i_y -> setText(text+'F');break;
		default:o_ans -> setText("Input Error");break;
	}
}
void TMainForm::knx_slot() {
	text_index = 1;
	QString text = i_x -> text();
	if (text[0] == '-') i_x -> setText(text.mid(1));
	else i_x -> setText('-' + text);
}
void TMainForm::kny_slot() {
	text_index = 2;
	QString text = i_y -> text();
	if (text[0] == '-') i_y -> setText(text.mid(1));
	else i_y -> setText('-' + text);
}
void TMainForm::slot_b() {
	QString text;
	switch (text_index) {
		case 1:text = i_x -> text();i_x -> setText(text.left(text.length() - 1));break;
		case 2:text = i_y -> text();i_y -> setText(text.left(text.length() - 1));break;
	}
}
...
void TMainForm::slot_i1() {text_index = 1;}
void TMainForm::slot_i2() {text_index = 2;}
void TMainForm::slot_o0() {
	op_index = 0, text_index = 2;
	o_ans -> setText("operator: (+) add , clicked <Get Answer> to get answer");
}
void TMainForm::slot_o1() {
	op_index = 1, text_index = 2;
	o_ans -> setText("operator: (-) subtract , clicked <Get Answer> to get answer");
}
void TMainForm::slot_o2() {
	op_index = 2, text_index = 2;
	o_ans -> setText("operator: (*) multiply , clicked <Get Answer> to get answer");
}
void TMainForm::slot_o3() {
	op_index = 3, text_index = 2;
	o_ans -> setText("operator: (/) divide , clicked <Get Answer> to get answer");
}
